...
<nav class="flex-1 overflow-y-auto py-4">
    <ul class="flex flex-col gap-2">
        ...
        <li x-data="{ open: false }">
            ...
            <div
                @click="open = !open"
                :class="(currentRoute === 'desensitization' || currentRoute === 'betalactam-cross-reactivity' || currentRoute === 'skin-test-concentrations' || currentRoute === 'desensitization-protocol-library') ? 'bg-primary/20' : 'hover:bg-slate-200/50 dark:hover:bg-slate-800/50'"
                class="flex h-12 items-center gap-4 rounded-lg px-4 cursor-pointer"
            >
                <span class="material-symbols-outlined" :class="(currentRoute === 'desensitization' || currentRoute === 'betalactam-cross-reactivity' || currentRoute === 'skin-test-concentrations' || currentRoute === 'desensitization-protocol-library') ? 'text-primary' : 'text-slate-600 dark:text-slate-400'">pill</span>
                <p :class="(currentRoute === 'desensitization' || currentRoute === 'betalactam-cross-reactivity' || currentRoute === 'skin-test-concentrations' || currentRoute === 'desensitization-protocol-library') ? 'text-primary' : 'text-slate-800 dark:text-slate-200'" class="text-base font-bold truncate flex-1">İlaç Alerjileri</p>
                <span class="material-symbols-outlined text-lg transition-transform" :class="[open ? 'rotate-180' : '', (currentRoute === 'desensitization' || currentRoute === 'betalactam-cross-reactivity' || currentRoute === 'skin-test-concentrations' || currentRoute === 'desensitization-protocol-library') ? 'text-primary' : 'text-slate-600 dark:text-slate-400']">expand_more</span>
            </div>
            ...
            <div x-show="open" x-collapse class="ml-4 mt-1 space-y-1">
                ...
                <a
                    ...
                    class="flex h-10 items-center gap-3 rounded-lg px-4 cursor-pointer"
                >
                    <span class="material-symbols-outlined text-lg" :class="currentRoute === 'desensitization-protocol-library' ? 'text-primary' : 'text-slate-600 dark:text-slate-400'">library_books</span>
                    <p :class="currentRoute === 'desensitization-protocol-library' ? 'text-primary' : 'text-slate-800 dark:text-slate-200'" class="text-sm font-medium truncate">Protokol Kütüphanesi</p>
                </a>
            </div>
        </li>
